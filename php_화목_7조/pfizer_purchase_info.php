<?php
	...
	$sql = "SELECT * FROM `systemdb`.`purchasing tbl` WHERE `purchasing tbl`.`purchasing_pharmaceuticalCom`='Pfizer'";
	...
	if($ret) {
		...
		echo "화이자 백신 구매내역 조회 실패". "<br>";
		echo "실패원인 : " . mysqli_error($con);
		exit();
	}

	echo "<h1> 화이자 백신 구매내역 조회 결과 </h1>";
	echo "<h3>","<a href = 'pfizer_purchase_info_insert.html'>구매내역 추가</a></h3>";
	...
	while($row = mysqli_fetch_array($ret)){
		...
		echo "<TD>", "<a href = 'pfizer_purchase_info_update.php?orderNumber=", $row['orderNumber']. "'>수정</a></TD>";
		echo "<TD>", "<a href = 'pfizer_purchase_info_delete.php?orderNumber=",$row['orderNumber']."'>삭제</a></TD>";
 		echo "</TR>";
