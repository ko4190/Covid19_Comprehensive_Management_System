...
<?php
 ...
 $sql ="SELECT * FROM systemdb.`people tbl` AS p LEFT JOIN systemdb.`patient tbl` AS pat ON p.`Resident registration number`=pat.`Resident registration number` LEFT JOIN systemdb.nation AS n ON n.Nation_ID=p.Nation_Nation_ID where `Vaccine number`='$Vaccine_number' AND `hospital tbl_Hospital Name`='Kyungpook'";
