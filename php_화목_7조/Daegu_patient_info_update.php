 ...
 $sql ="SELECT * FROM `systemdb`.`patient tbl` WHERE `patient tbl`.`Resident registration number` = '".$_GET['Resident_registration_number']."'";
   ...
   if($ret) {
	   ...
	   if ($count==0) {
		   ...
		   echo "<br> <a href='Daegu_main.html'> <--초기 화면</a> ";
		   exit();	
	   }		   	
   }
   else {
...
<h1> 환자 정보 수정 </h1>
<FORM METHOD="post"  ACTION="Daegu_patient_info_update_result.php">
	...
	백신 종류 : <INPUT TYPE ="text" NAME="COVID_19_confirmed" VALUE=<?php echo $COVID_19_confirmed ?>> <br>
	백신 접종 여부 : <INPUT TYPE ="text" NAME="Record_of_vaccination" VALUE=<?php echo $Record_of_vaccination ?>> <br>
 ...
 <br> <h3> <a href='patient_DH.php'> <--환자 조회 화면</a> </h3>
