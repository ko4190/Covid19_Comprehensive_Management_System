<?php
   ...
   $sql  =  "SELECT  * FROM   `systemdb`.`people tbl`  JOIN `systemdb`.`patient tbl` 
	...
	WHERE `hospital tbl_Hospital Name` = 'Yeungnam'";
...
echo "<TABLE border=1>";
echo "<TR>";
echo "<TH>주민번호</TH><TH>이름</TH><TH>주소</TH><TH>전화번호</TH><TH>입원일</TH><TH>퇴원일</TH><TH>사망일</TH><TH>코로나 검사일</TH><TH>확진상태</TH><TH>백신 종류</TH><TH>백신 1차 접종일</TH><TH>백신 2차 접종일</TH><TH>백신 부작용</TH><TH>백신 번호</TH></TH><TH>수정</TH><TH>삭제";
echo "</TR>";
while($row=mysqli_fetch_array($ret)){
	...
	echo "<TD>", "<a href='Yeungnam University_patient_info_update.php?Resident_registration_number=",$row['Resident registration number'],"'>수정</a></TD>";
	echo "<TD>", "<a href='delete_YH.php?Resident_registration_number=",$row['Resident registration number'],"'>삭제</a></TD>";
	echo "</TR>";
}

echo "</TABLE>";
echo "<br> <a href='patient_YH.php'> <--초기 화면</a> ";
