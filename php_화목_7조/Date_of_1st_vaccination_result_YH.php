...
<?php
     ...
     $Date_of_1st_vaccination = $_GET['Date_of_1st_vaccination'];

 $sql ="SELECT * FROM systemdb.`people tbl` AS p LEFT JOIN systemdb.`patient tbl` AS pat ON p.`Resident registration number`=pat.`Resident registration number` LEFT JOIN systemdb.nation AS n ON n.Nation_ID=p.Nation_Nation_ID where `Date of 1st vaccination`='$Date_of_1st_vaccination' AND `hospital tbl_Hospital Name`='Yeungnam'";
    ...
    $ret = mysqli_query($con, $sql);
      ...
      if($ret) {
       ...
       if ($count==0) {
         echo $_GET['Date_of_1st_vaccination']." 일치하는 백신 1차 접종일의 환자 없음!!!"."<br>";
...
echo "<TABLE>";
 echo "<h1> 환자 정보 조회 결과 </h1>";
     echo "<TABLE border=1>";
     echo "<TR>";
     echo "<TH>백신 1차 접종일</TH><TH>주민등록번호</TH><TH>이름</TH><TH>주소</TH><TH>전화번호</TH><TH>입원일</TH><TH>퇴원일</TH><TH>사망일</TH><TH>코로나 검사일</TH><TH>확진 상태</TH><TH>백신 종류</TH><TH>백신 2차 접종일</TH><TH>백신 부작용</TH><TH>백신 번호</TH>";
     echo "</TR>";
